...
; Try all encoding
; RUN: %jfs -cxx -branch-encoding=try-all --disable-equality-extraction --disable-standard-passes -libfuzzer-runs=100 -libfuzzer-seed=1 -stats-file=%t-stats_try-all.yml -record-max-num-satisfied-constraints -record-num-inputs -record-num-wrong-sized-inputs %s | %FileCheck -check-prefix=CHECK-SAT %s
